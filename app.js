...
function renderRoundSelector(rounds) {
    ...
    const render = (allRounds) => {
        ...
        allRounds.forEach(round => {
            ...
            btn.className = `round-selector-btn px-4 py-3 rounded-lg font-semibold shadow${selectedRoundId === round.round_id ? ' active' : ''}`;
            ...
            btn.innerHTML = `<div class="text-base font-bold">${course.course_name || 'Course'} <span class="text-xs font-normal text-gray-500">(${tee.tee_name || ''})</span></div>
                             <div class="text-xs text-gray-600">${correctDate}</div>
